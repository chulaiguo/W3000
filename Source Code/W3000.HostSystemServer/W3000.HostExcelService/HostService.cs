...
namespace W3000.HostExcelService
{
    public partial class HostService : ServiceBase
    {
        public HostService()
        {
            ...
            this.ServiceName = "W3000.ExcelService";
        }
        ...
        private void StartRemotingService()
        {
            try
            {
                Type basicServiceType = Type.GetType("Cheke.ExcelService.ExcelServiceFactory, Cheke.ExcelService");
                if (basicServiceType != null)
                {
                    RemotingConfiguration.RegisterWellKnownServiceType(basicServiceType,
                        "ExcelServiceFactory.soap", WellKnownObjectMode.SingleCall);
                }
                else
                {
                    this.WriteError("ExcelServiceFactory can't be start.");
                }
            }
            catch (Exception ex)
            {
        ...
        private void UpdateSelf()
        {
            try
            {
                const string projectName = "W3000.ExcelService";
