...
namespace W3000.HostDataService
{
    public partial class HostService : ServiceBase
    {
        public HostService()
        {
            ...
            this.ServiceName = "W3000.DataService";
        }
        ...
        private void StartRemotingService()
        {
            try
            {
                Type basicServiceType = Type.GetType("W3000.BasicService.BasicServiceFactory, W3000.BasicService");
                if (basicServiceType != null)
                {
                    RemotingConfiguration.RegisterWellKnownServiceType(basicServiceType,
                        "BasicServiceFactory.soap", WellKnownObjectMode.SingleCall);
                }
                else
                {
                    this.WriteError("BasicServiceFactory can't be start.");
                }
            }
            catch (Exception ex)
            {
