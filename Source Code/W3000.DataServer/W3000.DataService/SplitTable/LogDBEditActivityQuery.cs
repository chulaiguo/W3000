...
namespace W3000.DataService.SplitTable
{
    internal class LogDBEditActivityQuery
    {
        ...
        public LogDBEditActivityDataCollection GetBySqlWhere(string sqlWhere, SqlParameter[] paras)
        {
        ...
        public LogDBEditActivityDataCollection GetBySqlWhere(string where, SqlParameter[] paras, int pageIndex, int pageSize)
        {
            if (pageIndex == -1)
            {
                pageIndex = 0;
            ...
            foreach (KeyValuePair<string, LogRange> pair in index)
            {
                ...
                builder.AppendFormat("SELECT ROW_NUMBER() OVER({0}) as rowid, * FROM [{1}] as [{2}] WHERE {3} ",
                    sqlOrderBy, pair.Key, LogDBEditActivitySchema.TableAlias, sqlWhere);
                builder.Append(") T ");
                builder.AppendFormat("WHERE rowid BETWEEN {0} AND {1}", pair.Value.Begin, pair.Value.End);

                LogDBEditActivityCRUD crud = new LogDBEditActivityCRUD(this._dal.ConnectionString);
        ...
        {
            int index = 0;
            SortedList<DateTime, DateTime> dateIndex = HelperLog.SplitByMonth(this._begin, this._end);
            foreach (KeyValuePair<DateTime, DateTime> item in dateIndex)
            {
                string tableName = HelperLog.GetSplitByMonthTable(LogDBEditActivitySchema.TableName, item.Key.Year, item.Key.Month);
        ...
        private int GetSelectCount(string tableName, string sqlWhere, SqlParameter[] paras)
        {
            string sql = string.Format("SELECT COUNT(*) FROM {0} as {1} WHERE {2}",
                 tableName, LogDBEditActivitySchema.TableAlias, sqlWhere);
        ...
        private string GetOrderBySql()
        {
            ...
            {
                builder.AppendFormat("[{0}] ASC", LogDBEditActivitySchema.LogDateTime);
            }
        ...
        private string GetByDateTimeSql(DateTime begin, DateTime end, List<SqlParameter> paraList)
        {
            string sql = string.Format(" [{0}].[{1}] >= @Min{1} AND [{0}].[{1}] <= @Max{1} ",
                LogDBEditActivitySchema.TableAlias, LogDBEditActivitySchema.LogDateTime);

            SqlParameter para = new SqlParameter(string.Format("@Min{0}", LogDBEditActivitySchema.LogDateTime), SqlDbType.DateTime);
            para.Value = begin;
            paraList.Add(para);

            para = new SqlParameter(string.Format("@Max{0}", LogDBEditActivitySchema.LogDateTime), SqlDbType.DateTime);
            para.Value = end;
            paraList.Add(para);
